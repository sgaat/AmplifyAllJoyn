...
@SuppressLint("NewApi") public class AllJoynService extends Service implements Observer {
				...
				if (mJoinedToSelf) {
					if (mHostChatInterface != null) {
						if ( message.messageType == 0 ){
							mHostChatInterface.Chat(message.message);
						}else if ( message.messageType == 1 ){
							mHostChatInterface.PlotPoint(message.xCord, message.yCord, message.xOldCord, message.yOldCord, message.color);
						}else if ( message.messageType == 2 ){
							mHostChatInterface.ClearCanvas();
					...
					if ( message.messageType == 0 ){
						mChatInterface.Chat(message.message);
					}else if ( message.messageType == 1 ){
						mChatInterface.PlotPoint(message.xCord, message.yCord, message.xOldCord, message.yOldCord, message.color);
					}else if ( message.messageType == 2 ){
						mChatInterface.ClearCanvas();
					}else if ( message.messageType == 3 ){
	...
	class ChatService implements ChatInterface, BusObject {
		...
		public void PlotPoint(double xCord, double yCord, double xOldCord, double yOldCord, long color) throws BusException {
		}

		public void ClearCanvas() throws BusException {
		}

		public void SetCanvasColor(long color, int pointerPosition)	throws BusException {
		}
	 ...
	 * capitalized to conform with the DBus convention for signal handler names.
	 */
	@BusSignalHandler(iface = "org.alljoyn.bus.samples.chat", signal = "PlotPoint")
	public void PlotPoint(double xCord, double yCord, double xOldCord,	double yOldCord, long color) {
		...
		MessageObject mo = new MessageObject();
		mo.messageType = 1;
		mo.xCord = (float) xCord;
		mo.xOldCord = (float) xOldCord;
		mo.yCord = (float) yCord;
		mo.yOldCord = (float) yOldCord;
		mo.color = color;
		mChatApplication.newRemoteUserMessage(nickname, mo);
	}
	 ...
	 * capitalized to conform with the DBus convention for signal handler names.
	 */
	@BusSignalHandler(iface = "org.alljoyn.bus.samples.chat", signal = "ClearCanvas")
	 ...
	 * capitalized to conform with the DBus convention for signal handler names.
	 */
	@BusSignalHandler(iface = "org.alljoyn.bus.samples.chat", signal = "SetCanvasColor")
	public void SetCanvasColor(long color, int pointerPosition){
		...
		mo.messageType = 3;
		mo.color = color;
		mo.pointerPosition = pointerPosition;
		mChatApplication.newRemoteUserMessage(nickname, mo);
	}
	 ...
	 * capitalized to conform with the DBus convention for signal handler names.
	 */
	@BusSignalHandler(iface = "org.alljoyn.bus.samples.chat", signal = "CompressPlots")
	public void CompressPlots(String points){
